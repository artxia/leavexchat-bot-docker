...
RUN apt-get update && apt-get install --yes git curl \
    ...
    && cd /leavexchat-bot \
    && sed -i 's/"wechaty": "^0.56.6"/"wechaty": "^0.59.8"/g' package.json \
    && sed -i 's/"wechaty-puppet-puppeteer": "0.24.6"/"wechaty-puppet-wechat": "^0.27.0"/g' package.json \
    && sed -i 's/wechaty-puppet-puppeteer/wechaty-puppet-wechat/g' src/Bot.ts \
    && npm i \
    && npm run build \
    && rm -fr /tmp/* ~/.npm
